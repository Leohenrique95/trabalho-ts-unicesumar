...
async function executarExercicio09() {
    console.log("=== CALCULADORA ===");
    const entrada1 = await (0, input_1.pergunta)("Digite o primeiro número: ") ?? "";
    ...
    while (!["+", "-", "*", "/"].includes(operador)) {
        operador = await (0, input_1.pergunta)("\nDigite o operador: ") ?? "";
        if (!["+", "-", "*", "/"].includes(operador)) {
            console.log("Operador inválido.");
        }
    }
    const entrada2 = await (0, input_1.pergunta)("Digite o segundo número: ") ?? "";
