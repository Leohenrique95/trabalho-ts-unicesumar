...
export async function executarExercicio07() {
    let numerosInteiros: number[] = [];

    console.log(" Encontrando o maior numero no array ");

    for (let i: number = 0; i < 5; i++) {
        let resposta: string = await pergunta(`Digite o ${i + 1}o numero inteiro: `) ?? "";
        let numeroConvertido: number = parseInt(resposta);
    ...
    let numeroMaximo: number = numerosInteiros[0];

    for (let i: number = 1; i < numerosInteiros.length; i++) {
        let valorAtual: number = numerosInteiros[i];
